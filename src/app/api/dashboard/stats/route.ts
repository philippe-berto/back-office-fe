...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Proxying dashboard stats request to: ${backendURL}/api/dashboard/stats`);
    ...
    return NextResponse.json(
      { error: 'Dashboard stats service unavailable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}