...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Proxying sessions request to: ${backendURL}/api/sessions`);
    ...
    return NextResponse.json(
      { error: 'Sessions service unavailable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}