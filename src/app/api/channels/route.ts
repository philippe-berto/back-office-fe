...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Proxying channels request to: ${backendURL}/api/channels`);
    ...
    return NextResponse.json(
      { error: 'Channels service unavailable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}